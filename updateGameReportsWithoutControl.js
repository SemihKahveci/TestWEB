...
// MongoDB URI'yi doğrudan tanımla
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://hasansemihkahveci:HHwoTrWoMuvHQeG2@admin-paneli-cluster.qypdg.mongodb.net/admin-paneli-cluster?retryWrites=true&w=majority&appName=admin-paneli-cluster';
...
class GameReportUpdater {
    ...
    async updateGameReports(game) {
        try {
            ...
            // Eski ve yeni raporları karşılaştır (sadece bilgi için)
            ...
            // Her zaman güncelle (ID kontrolü yapma)
    ...
    async updateSpecificCode(playerCode) {
        try {
            ...
            // Debug: Tüm oyunları listele
            const allGames = await Game.find({});
            console.log(`🔍 Toplam oyun sayısı: ${allGames.length}`);
            
            // Debug: PlayerCode'ları listele
            const playerCodes = allGames.map(g => g.playerCode);
            console.log(`🔍 Mevcut kodlar: ${playerCodes.slice(0, 10).join(', ')}...`);

            const games = await Game.find({ playerCode });
            
            if (games.length === 0) {
                console.log('❌ Bu kod için oyun bulunamadı');
                console.log(`🔍 Aranan kod: ${playerCode}`);
                console.log(`🔍 Mevcut kodlar: ${playerCodes.slice(0, 5).join(', ')}...`);
                return;
            }
