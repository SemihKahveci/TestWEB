...
const scriptFilesController = {
    // Dosya yükleme (sadece geçici olarak kaydet, DB'ye yazma)
    uploadScriptFile: async (req, res) => {
        ...
        try {
            ...
            safeLog('info', `Dosya geçici olarak kaydedildi: ${tempCsvPath}`, {
            ...
            res.json({
                success: true,
                message: `Dosya başarıyla yüklendi. Güncelleme butonlarına basarak işlemi tamamlayabilirsiniz.`,
                data: {
                    competencyName: competencyName,
                    fileName: file.originalname
                }
            ...
            safeLog('error', 'Dosya yükleme hatası', error);
            res.status(500).json({
                success: false,
